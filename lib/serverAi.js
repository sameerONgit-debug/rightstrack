...
function languageInstruction(language, subject) {
  const languageName = LANGUAGE_NAMES[language] || LANGUAGE_NAMES.en;
  if (language === 'en') return `Write the ${subject} in clear conversational English.`;
  return `Write the ${subject} in clear ${languageName}. Use the selected language for all user-facing text, while keeping domain values, field keys, IDs, JSON property names, act names, section numbers, and statutory citations unchanged.`;
}
...
export async function draftCase({ analysis, answers, language = 'en' }) {
  ...
  try {
    const languageInstructionText = language === 'en'
      ? 'Draft the official document in formal English legal notice or petition format.'
      : `Draft the official document in formal ${LANGUAGE_NAMES[language] || 'selected-language'} legal petition format, or a clear bilingual format. Preserve statutory act names, section numbers, and citations exactly.`;
    const result = parseJson(await callProvider(`${languageInstructionText} The analysis, answers, and original facts may contain English, Hindi, Marathi, Bengali, Tamil, or mixed Roman/Indic script. Preserve their meaning and do not fail on mixed-language text. Preserve the user's facts, do not invent missing facts, and use only citations supplied in the analysis. ${schema}\n\nAnalysis:\n${JSON.stringify(analysis)}\n\nAnswers:\n${JSON.stringify(answers)}`));
