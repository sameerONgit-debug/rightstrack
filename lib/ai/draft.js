...
async function buildDraft({ domain, fields = {}, retrievedChunks = [], narrative = '', answers = {} }) {
  ...
  try {
    guidance = await generateGuidance({
      ...
      domain,
      category: domain === 'Unsupported' ? 'AI-generated grievance category' : domain,
      fields,
      answers,
      retrievedChunks,
    });
  ...
  try {
    ...
    const raw = await callClaude({
      ...
      userPrompt: JSON.stringify({ domain, narrative, extracted_fields: fields, answers, guidance, retrieved_legal_sources: sourcePack }),
      temperature: 0.1,
      maxTokens: 3600,
    });
