...
async function callGemini({ systemPrompt, userPrompt, temperature = 0.2, maxTokens = 2000 }) {
  ...
  // Keep the model configurable, but use a currently supported Gemini model by default.
  // Do not retry retired model IDs: a bad model should produce one clear provider error.
