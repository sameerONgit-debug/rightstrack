/**
 * RightsTrack semantic case-understanding layer.
 *
 * Gemini is authoritative by default. Deterministic extraction/question
 * fallbacks are opt-in for normal callers, while the top-level analyzer may
 * explicitly fail open during a provider outage.
 */
...
function fallbackAllowed() {
  return String(process.env.AI_ALLOW_FALLBACK).toLowerCase() === 'true';
}
...
async function extract(text, domain) {
  ...
  try {
    ...
    const raw = await callClaude({
      systemPrompt: `You are RightsTrack's semantic case-fact extractor for Indian civic and legal assistance. Read the citizen's COMPLETE narrative regardless of wording, spelling, grammar or vocabulary. Understand relationships and chronology semantically. Do not use keyword matching and do not guess. Extract only facts explicitly stated or unambiguously implied. Missing values MUST be null. Preserve important actors, dates, amounts, locations, harm and desired remedy.\n\nMatter type: ${domain}\nAllowed fields: ${SCHEMAS[extractionDomain].join(', ')}\n\nReturn ONLY valid JSON with exactly those fields.`,
      userPrompt: String(text).trim(), temperature: 0, maxTokens: 1200,
    });
    if (!raw) throw new Error('AI returned no extracted facts.');
    return sanitizeFields(parseJson(raw), extractionDomain);
  } catch (err) {
    console.warn('[Extractor] AI extraction failed:', err?.message || err);
    if (fallbackAllowed()) return heuristicExtract(text, extractionDomain);
    throw new Error(`AI fact extraction unavailable: ${err?.message || 'provider failure'}`);
  }
}
...
async function generateClarifyingQuestions({ narrative, domain, fields }) {
  try {
    const { callClaude } = await import('./claude.js');
    const raw = await callClaude({
      systemPrompt: `You are RightsTrack's adaptive legal-intake interviewer for India. Generate the smallest useful set of follow-up questions for THIS exact grievance.\n\nReason from the actual facts, actors, chronology, harm, desired outcome and jurisdiction. Do not use a fixed questionnaire. Do not ask for information already present. Prioritize facts that can change legal applicability, responsible authority, jurisdiction, deadlines/limitation, evidence, amount/harm and remedy. Ask at most 6 questions; ask fewer when enough is already known. If a precise authority/office requires location, ask for the relevant state and district/city.\n\nEach question must be answerable by an ordinary citizen and must explain one missing fact, not ask several unrelated things at once.\n\nReturn ONLY JSON: {"questions":[{"id":"stable_short_id","question":"...","input_type":"text|number|date","required":true}]}\n\nMatter type: ${domain}\nKnown facts: ${JSON.stringify(fields)}`,
      userPrompt: narrative, temperature: 0.15, maxTokens: 1000,
    });
    if (!raw) throw new Error('AI question generation unavailable.');
    const parsed = parseJson(raw);
    return Array.isArray(parsed.questions) ? parsed.questions.filter(q => q?.question).slice(0, 6).map((q, i) => ({
    ...
    console.warn('[Questioner] AI question generation failed:', err?.message || err);
    if (fallbackAllowed()) return clarifyingQuestions(domain, fields);
    throw new Error(`AI question generation unavailable: ${err?.message || 'provider failure'}`);
  }
}
...
function clarifyingQuestions(domain, fields = {}) {
  ...
  return (questions[domain] || []).filter(([key]) => !fields?.[key]).map(([field_key, question_text, input_type]) => ({ field_key, question_text, input_type, ai_generated: false }));
}

module.exports = { extract, generateClarifyingQuestions, clarifyingQuestions, heuristicExtract, SCHEMAS };
