...
function extract(text, domain) {
  ...
  if (domain === 'RTI') {
    return {
      authority_name: firstMatch(value, /(?:office|department|authority|block office)\s+(?:at|of|named)?\s*([^,.]+)/i),
      information_requested: firstMatch(value, /(?:want|need|seeking|requesting)\s+(?:to know|information about|information on)?\s*([^,.]+)/i),
      applicant_name: null,
  ...
  return {
    merchant_name: firstMatch(value, /(?:seller|merchant|company)\s+(?:is|:)?\s*([^,.]+)/i),
    product_or_service: firstMatch(value, /(?:bought|purchased|ordered)\s+(?:a|an)?\s*([^,.]+)/i),
    ...
    issue: firstMatch(value, /(?:problem|issue|defect|damaged|cracked|broken)\s*(?:is|was|:)?\s*([^,.]+)/i),
